...
def main():
    ...
    if "current_query_movies" not in st.session_state:
        st.session_state.current_query_movies = []
    if "current_query_to_show" not in st.session_state:
        st.session_state.current_query_to_show = 20
    ...
    with col1:
        if st.button("📋 Show All Movies", key="quick_show_all"):
    ...
    with col2:
        if st.button("🔍 Top Rated Movies", key="quick_top_rated"):
    ...
    with col3:
        if st.button("🎭 Action Movies", key="quick_action"):
    ...
    with col4:
        if st.button("📊 Movie Statistics", key="quick_stats"):
    ...
    # Display current query results with pagination
    if st.session_state.current_query_movies:
        st.subheader("🔍 Query Results")
        query_movies_to_display = st.session_state.current_query_movies[:st.session_state.current_query_to_show]
        
        st.write(f"**Found {len(st.session_state.current_query_movies)} movies:**")
        for i, movie in enumerate(query_movies_to_display):
            st.write(f"{i+1}. **{movie['title']}** ({movie['year']}) - ⭐ {movie['rating']}")
        
        if len(st.session_state.current_query_movies) > st.session_state.current_query_to_show:
            remaining = len(st.session_state.current_query_movies) - st.session_state.current_query_to_show
            if st.button(f"📥 Show More Results ({remaining} remaining)", key="show_more_query"):
                st.session_state.current_query_to_show += 20
                st.rerun()
    ...
    # Always show chat input for new queries
    if not prompt:
...
"""
                ...
                elif "least rated" in prompt_lower or "lowest rated" in prompt_lower or "worst rated" in prompt_lower:
                    graphql_query = "query { movies { id title year rating } }"
                ...
                elif "game-show" in prompt_lower:
                    graphql_query = 'query { searchMovies(genre: ["Game-Show"]) { id title year genre rating } }'
                ...
                if "errors" in result:
                    ...
                    if not data or all(not v for v in data.values()):
                        ...
                        if "movies" in data and data["movies"]:
                            movies = data["movies"]
                            if len(movies) > 0:
                                # Sort movies based on query type
                                prompt_lower = prompt.lower()
                                ...
                                # Store movies in session state for pagination
                                st.session_state.current_query_movies = movies
                                st.session_state.current_query_to_show = 20
                                ...
                                if len(movies) > 10:
                                    response_text += f"\n... and {len(movies) - 10} more movies! (Use 'Show More Results' button below)"
    ...
    # Example queries section below chat
    st.markdown("---")
    st.markdown("**💡 Try these example queries:**")
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("🎬 Show me all movies", key="example1"):
            st.session_state.example_query = "Show me all movies"
        if st.button("🔍 Find movies by Christopher Nolan", key="example2"):
            st.session_state.example_query = "Find movies by Christopher Nolan"
        if st.button("⭐ What are the highest rated movies?", key="example3"):
            st.session_state.example_query = "What are the highest rated movies?"
    
    with col2:
        if st.button("🎭 Show me action movies", key="example4"):
            st.session_state.example_query = "Show me action movies"
        if st.button("📅 Movies from 2008", key="example5"):
            st.session_state.example_query = "Movies from 2008"
        if st.button("🏆 Movies with rating above 8", key="example6"):
            st.session_state.example_query = "Movies with rating above 8"
    
    with col3:
        if st.button("🎬 Movies by Steven Spielberg", key="example7"):
            st.session_state.example_query = "Movies by Steven Spielberg"
        if st.button("💥 Comedy movies", key="example8"):
            st.session_state.example_query = "Comedy movies"
        if st.button("📊 Movie statistics", key="example9"):
            st.session_state.example_query = "Movie statistics"
